...
public class MybatisLogInterceptor implements Interceptor {
	...
	private final static String LOG_FILE_DIR = "d:\\MBI_SQL_LOG\\"; 
	private static SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
	private static SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH");
	private static SimpleDateFormat udf = new SimpleDateFormat("yyyy년MM월dd일 HH시 mm분 ss초"); 
	...
	// TODO Auto-generated method stub
        ...
        if(param == null){				// 파라미터가 아무것도 없을 경우
        	...
        	for(ParameterMapping mapping : paramMapping){
	        	    ...
	        	    if(String.class == javaType){				// SQL의 ? 대신에 실제 값을 넣는다. 이때 String 일 경우는 '를 붙여야 하기땜에 별도 처리
	        	    	...
	        	    	if ( field == null || field.get(param) == null )
	        	    		sql = sql.replaceFirst("\\?", "null");
	        	    	else
        ...
        try{
        	rtObj = invocation.proceed();
        }catch(Exception e){
        	resultException = e;
        	//e.printStackTrace();
        	StackTraceElement[] steArr = e.getCause().getStackTrace();
        	errorLog += "\n\n\tat ";
        	errorLog += e.getCause().toString();
        	errorLog += "\n";
        	for(StackTraceElement steObj : steArr) {
        	  errorLog += "\tat ";
        	  errorLog += steObj.toString();
        	  errorLog += "\n";
      	   }
        }finally{
        	
        	if(!(sql.indexOf("SW_MBI_LOG_HIST") > -1)){
        		
        		long endSql = System.currentTimeMillis();
        		double sqltime = endSql-startSql;
            	FileWriter fstream = null;
    			try {
    				//File dir = new File(LOG_FILE_DIR+df.format(new java.util.Date()));
    				//if(!dir.exists()){
    				//	dir.mkdirs();
    				//}
    				//fstream = new FileWriter(LOG_FILE_DIR+df.format(new java.util.Date())  + "\\" + sdf.format(new java.util.Date()) + ".txt", true);
    				//BufferedWriter out = new BufferedWriter(fstream);
    	        
    	        	String sqlLog = "";
    	        
    	        	sqlLog += "=======================================================================================================\r\n";
    	        	sqlLog += " - 실행일자: " +udf.format(new java.util.Date())+"\r\n";
    	        	sqlLog += " - CLIENT: " +java.net.Inet4Address.getLocalHost().getHostAddress() +"\r\n";
    	        	sqlLog += "=======================================================================================================\r\n";
    	        	sqlLog += "\r\n";
    	        	sqlLog += "[ SQL : Input Parameter ]" + "\r\n";
    	        	sqlLog += new Gson().toJson(param).toString()  + "\r\n";  
    	    		sqlLog += "\r\n";
    	    		sqlLog += "[ SQL : Excute ]" + "\r\n";
    	    		sqlLog += sql;
    	    		sqlLog += "\r\n"; 
    	    		...
    	    		if(errorLog == ""){
    	    			sqlLog += "[ 실행시간 ]  " + sqltime/1000 +" sec."+ "\r\n";
    	    		
    	    		}else{
    	    			sqlLog += "SQL Error Occured.....";
    	    			sqlLog += errorLog;
    	    		}
