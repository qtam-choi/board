...
public class BoardFileUtil {
     ...
     * 파일 업로드.
     */
    public List<BoardFileVO> saveAllFiles(List<MultipartFile> upfiles) {
    	
    	String saveDirPath = File.separator + "board" +File.separator +"file" + File.separator + DateUtil.getToday("yyyyMM") + File.separator ;



    	String filePath =  saveDirPath;
        ...
        for (MultipartFile uploadfile : upfiles ) {
            ...
            BoardFileVO filedo = new BoardFileVO();
    ...
    public String getRealPath(String path, String filename) {
        return path + filename.substring(0,4) + File.separator;
    }
}
